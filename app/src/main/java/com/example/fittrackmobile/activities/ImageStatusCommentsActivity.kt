...
/**
 * Activity to display comments for an image status.
 */
class ImageStatusCommentsActivity : AppCompatActivity() {
    ...
    // Class variables.
    private var TAG = "ImageStatusCommentsActivity"
    ...
    private fun addCommentToRecyclerView() {
        try {
            val queryComments = firebaseFirestore.collection("ImageStatus")
                        ...
                        .addOnSuccessListener { documentSnapshot ->
                            ...
                            firebaseFirestore.collection("ImageStatus")
                                ...
                                .addOnSuccessListener {
                                    ...
                                    Toast.makeText(this@ImageStatusCommentsActivity,
                                    ...
                                    addNotifications.generateNotification(currentUser,
                                        "commented on", "image status",
                                        receivedUserEmail)
                                    commentET.setText("")

                                    // Get a collection reference of the comments of the current status.
                                    collectionReference = firebaseFirestore.collection("ImageStatus")
                                        .document(documentID)
                                        .collection("Comments")
                                    ...
                                    collectionReference.get().addOnSuccessListener { querySnapshot ->
                                        ...
                                        val textStatusDocumentReference = firebaseFirestore.collection("ImageStatus")
                                            .document(documentID)
    ...
    private fun attachJavaToXML() {
        try {
            ...
            firebaseFirestore = FirebaseFirestore.getInstance()
            recyclerView = findViewById(R.id.image_status_comments_RV)
            commentET = findViewById(R.id.image_status_comments_commentET)
            addCommentBtn = findViewById(R.id.image_status_comments_AddCommentBtn)
        } catch (e: Exception) {
            Log.e(TAG, "" + e.message)
        }
    }
